...
namespace SmartSchoolsV2.Views
{
    ...
    public partial class VerifyAccountPage : ContentPage
    {
        ...
        public ICommand TapResend => new Command(async () => await CreatePassword());
        public VerifyAccountPage(string mobile_number, string username, string password)
        {
            InitializeComponent();
            BindingContext = this;
        ...
        async Task CreatePassword()
        {
            if (conn.IsConnected() == true)
            {
                try
                {
                    ...
                    var t = srvc.PostUserCreatePassword(_mobile_number, _username, _password, _password);
        ...
        async Task VerifyAccount(string otp)
        {
            if (conn.IsConnected() == true)
            {
                try
                {
                    ...
                    var t = srvc.PostAccountVerify(_mobile_number, _username, otp);
                    string jsonStr = await t;
                    CrudProperty response = JsonConvert.DeserializeObject<CrudProperty>(jsonStr);
                    if (response.Success == true)
                    {
