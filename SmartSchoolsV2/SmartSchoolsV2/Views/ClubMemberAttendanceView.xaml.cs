...
namespace SmartSchoolsV2.Views
{
    ...
    public partial class ClubMemberAttendanceView : ContentView
    {
        Connection conn = new Connection();
        ServiceWrapper srvc = new ServiceWrapper();
        public string requestUrl = Settings.requestUrl;
        CultureInfo ci = new CultureInfo(Settings.cultureInfo);
        public static Command LoadClubMemberAttendanceMonthly { get; set; }
        ...
        public ClubMemberAttendanceView()
        {
            InitializeComponent();
            this.BindingContext = this;

            LoadClubMemberAttendanceMonthly = new Command(async () => await ClubMemberMonthlyAttendance());
        }
        ...
        async void OnLeftArrowClicked(object s, DateTimeEventArgs e)
        {
            ...
            await ClubMemberMonthlyAttendance();
        }
        ...
        async void OnDateClicked(object s, DateTimeEventArgs e)
        {
            ...
            await ClubMemberDailyAttendance();
        }
        public async Task ClubMemberMonthlyAttendance()
        {
            if (conn.IsConnected() == true)
            {
                try
                {
                    ...
                    var t = srvc.PostSchoolClubMemberMonthlyAttendance(Settings.selectedSchoolId, Settings.selectedClubId, _profile_id, _monthYear.ToString("MM-yyyy"));
                    string jsonStr = await t;
                    ClubMemberMonthlyAttendanceProperty response = JsonConvert.DeserializeObject<ClubMemberMonthlyAttendanceProperty>(jsonStr);
                    if (response.Success == true)
                    {
                        string _dot = "";
                        var special = new List<SpecialDate>();
                        foreach (ClubMemberMonthlyAttendance sl in response.Data)
                        {
        ...
        public async Task ClubMemberDailyAttendance()
        {
            if (conn.IsConnected() == true)
            {
                try
                {
                    //ShowLoadingPopup();
                    var t = srvc.PostSchoolClubMemberDailyAttendance(Settings.selectedSchoolId, Settings.selectedClubId, _profile_id, _selectedDate.ToString("dd-MM-yyyy"));
                    string jsonStr = await t;
                    ClubMemberDailyAttendanceProperty response = JsonConvert.DeserializeObject<ClubMemberDailyAttendanceProperty>(jsonStr);
                    if (response.Success == true)
                    {
                        if (response.Data.Count > 0)
                        {
                            foreach (ClubMemberDailyAttendance sl in response.Data)
                            {
